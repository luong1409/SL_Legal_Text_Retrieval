...
nlp_file = "/vncorenlp_data/VnCoreNLP-1.1.1.jar"

nlp_file = "/".join(temp[:temp.index("Legal_Text_Retrieval") + 1] + nlp_file.split("/"))

tokenizer_obj = VnCoreNLP(nlp_file, annotators="wseg", max_heap_size='-Xmx500m') 
...
if __name__ == "__main__":
    ...
    _, (_, _, _, vectorizer) = do_classify(c_docs, c_keys, train_q,
                                                        stopwords_=stopwords, topk=topk_select, tokenizer=tokenizer, combine_score=(not options.tok))
    ...
    gen_mrpc_data(new_dataset_train, "{}/train.csv".format(path_folder_data_out))
